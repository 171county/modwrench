...
- **Found a bug?** â†’ [File a bug report](../../issues/new?template=bug_report.md)
- **Want a feature?** â†’ [File a feature request](../../issues/new?template=feature_request.md)
- **Want a platform supported?** â†’ [File a platform request](../../issues/new?template=platform_request.md)
- **Just have a question?** â†’ [Start a Discussion](../../discussions)
...
- Any error message in full (redact API keys if they appear â€” they shouldn't, but always check)
...
- `incompatible` â€” game crashes or major systems break when both are loaded
- `load-order-sensitive` â€” works if loaded in a specific order; needs documentation
- `patch-available` â€” known to conflict, but a community patch resolves it
- `informational` â€” interaction worth noting but doesn't break gameplay
...
Each platform is its own package (`@modwrench/<platform>`). The shape is documented in `docs/adding-a-platform.md` *(coming soon â€” file an issue if you want to take this on and the doc isn't there yet)*.

Strong candidate platforms (in rough order of community demand):

- Nexus, mod.io, and Thunderstore are already shipped read-side — see [packages/](packages/)
...
Documentation improvements are always welcome. Typo fixes, clarifications, examples â€” all welcome PRs. Don't ask permission, just open the PR.
...
ModWrench has six non-negotiables baked into the architecture. PRs that violate any of them won't be merged â€” not because the contributor is doing something wrong, but because these rules are the project's foundation.
...
You'll need Node.js 20+. ModWrench is intentionally portable â€” no Windows-specific dependencies, no native binaries that block Linux builds.
...
4. **Sign your commits.** ModWrench uses the Developer Certificate of Origin (DCO) instead of a CLA â€” see below.
5. **Update docs** if your change affects user-visible behavior.

### The DCO sign-off

ModWrench uses the [Developer Certificate of Origin](https://developercertificate.org/) â€” a lightweight alternative to a CLA. By signing your commits with `-s`, you're affirming that you have the right to contribute the code under the project's license.
...
- **Merge as-is** â€” straightforward, fits the project
- **Suggested changes** â€” minor adjustments, usually mechanical
- **Discussion needed** â€” the change touches architecture or one of the six non-negotiables; let's talk in the PR comments
- **Decline with reason** â€” rare but possible if the change conflicts with project direction

I aim to never leave a PR unanswered for more than a week. If I do, please nudge me â€” assume good intent (I probably just missed the notification).
...
- **Be a wrench, not a hammer.** Modders have been burned by enough hammers â€” extractive platforms, predatory monetization, AI generators that strip credit. ModWrench is supposed to be the antithesis of those. Carry that energy into PR reviews, issue threads, and Discussions.
...
Contributors are credited in commit history (every merged PR appears under your name in `git log`) and in release notes once we ship to npm. A standalone `CONTRIBUTORS.md` will be added when the contributor count makes it useful â€” likely via [all-contributors](https://allcontributors.org/) automation. For now `git shortlog -sne` is the source of truth.

If you contributed something significant and don't see your name, file an issue â€” it's an oversight, not a slight.

---

## License

By contributing, you agree that your contributions will be licensed under [MIT License](./LICENSE) â€” the same license as the rest of the project. The DCO sign-off on each commit is your affirmation of this.
